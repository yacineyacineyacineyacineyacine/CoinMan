...
public class CoinMan extends ApplicationAdapter {
	...
	public void render () {
		...
		if(gameState == 1){
			...
			for (int j = 0; j< bombsX.size(); j++){
				batch.draw(bomb, bombsX.get(j), bombsY.get(j));
				bombsX.set(j, bombsX.get(j) - 8);
				bombRectangles.add(new Rectangle(bombsX.get(j), bombsY.get(j), bomb.getWidth(), bomb.getHeight()));
			}
